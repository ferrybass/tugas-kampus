...
<form method="post" action="">
	<table>
		...
		<tr>
			<td>Tahun Ajaran</td>
			<td><input type="text" name="tahunajaran" value="2018/2019" readonly /></td>		
		</tr>
		<tr>
			<td>Biaya SPP</td>
			<td><input type="text" name="biaya" value="150000" /></td>		
		</tr>
		<tr>
			<td>Jatuh Tempo pertama</td>
			<td><input type="text" name="jatuhtempo" value="2018-07-10"  /></td>		
		</tr>
		<tr>
			<td></td>
...
<?php
	...
	$awaltempo 			= $_POST['jatuhtempo'];
		...
		// Membuat Array untuk menampung bulan bahasa Indnesia
		$bulaninfo = array(
			'01' => 'Januari',	
			'02' => 'Februari',	
			'03' => 'Maret',	
		...
		if($nis=='' || $nama=='' || $kelas==''){
			...
			if(!$simpan){
				...
				$ds=mysqli_fetch_array(mysqli_query($konek,"SELECT idsiswa FROM siswa ORDER by idsiswa DESC LIMIT 
					1"));
				$idsiswa = $ds['idsiswa'];
			...
			for($i=0; $i<12; $i++){
				...
				$bulan = $bulanInfo[date('m',strtotime($jatuhtempo))]."".date('Y',strtotime($jatuhtempo));
				mysqli_query($konek, "INSERT INTO spp(idsiswa,jatuhtempo,bulan,jumlah)
					value('$idsiswa','$jatuhtempo','$bulan','$biaya')");
				}
